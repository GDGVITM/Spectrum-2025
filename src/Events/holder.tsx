      ...
      <div className="absolute top-30 left-0">
        ...
        <div>
          <img
            ...
            alt="event banner"
          />
          <div className="absolute top-50 left-30 text-xl flex font-">
            <h1 className="mr-6">{selectedEvent.title}</h1>
            <Arrow width={80} height={80} />
          </div>
          <div className="w-[85%] absolute top-75 left-30 text-xl">
            {selectedEvent.description}
          </div>
          <div className="w-[85%] absolute top-110 left-30 flex flex-col">
            <span className="mb-10 text-5xl">Date: {selectedEvent.date}</span>
            <div className="flex justify-between">
              ...
              <div className="relative flex overflow-hidden p-4 w-[50%]">
                <div
                  className="flex overflow-x-auto space-x-4 p-4 w-full scroll-smooth snap-x snap-mandatory"
                ...
                >
                  {events.map((event) => (
                    <img
                      key={event.id}
                      src={event.image}
                      className={`w-[200px] h-[100px] cursor-pointer transition-transform duration-300 snap-center ${
